...
class Vehicle(ABC):
    ...
    def setSecondDestination(self, road: Road, position: float):
        self.secondDestinationRoad = road
        self.secondDestinationPosition = position
        realPositionX = (road.endPosReal[0] - road.startPosReal[0]) * position + road.startPosReal[0]
        realPositionY = (road.endPosReal[1] - road.startPosReal[1]) * position + road.startPosReal[1]
        self.secondDestinationRealPosition = (realPositionX, realPositionY)
    ...
    def __deepcopy__(self, memo):
        ...
        if hasattr(self, 'secondDestinationRoad'):
            agentCopy.setSecondDestination(self.secondDestinationRoad, self.secondDestinationPosition)
