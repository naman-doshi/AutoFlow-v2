...
def computeRoutes(selfish_vehicles: list[Vehicle], autoflow_vehicles: list[Vehicle], landscape: Landscape, MAX_ROAD_SPEED_MPS: float, receiveNewDests, carPositions = {}):
    """
    Compute the routes for selfish vehicles first, then AutoFlow vehicles.
    """
    routes = {}
    selfish_vehicle_routes = computeSelfishVehicleRoutes(selfish_vehicles, landscape, MAX_ROAD_SPEED_MPS, receiveNewDests, carPositions=carPositions)
    autoflow_vehicle_routes = computeAutoflowVehicleRoutes(autoflow_vehicles, landscape, MAX_ROAD_SPEED_MPS, receiveNewDests, carPositions=carPositions)
...
def computeSelfishVehicleRoutes(selfish_vehicles: list[Vehicle], landscape: Landscape, MAX_ROAD_SPEED_MPS: float, receiveNewDests, carPositions={}) -> list[list[tuple[float, float]]]:
    """
    ...
    for vehicle in selfish_vehicles:
        ...
        if receiveNewDests == 1:
            ...
            # Calculate real destination position (2nd dest)
            destination_position = vehicle.secondDestinationRealPosition

            # Nodes are the starting points of each road, can also be the starting point of the vehicle
            open_nodes: list[float, float, float, Road, float] = [] # Open is a priority queue
            closed_nodes: set[tuple[Road, float]] = set() # Closed can just be a set

            # Calculate the cost variables of the starting position (1st dest)
            gcost = 0
            hcost = manhattanDistance(
                vehicle.destinationRealPosition, 
                destination_position
            ) / MAX_ROAD_SPEED_MPS
            fcost = gcost + hcost

            # Add starting position (1st destination) of the vehicle to open_nodes
            start_node = (fcost, hcost, gcost, tiebreaker, vehicle.destinationRoad, vehicle.destinationPosition)
            tiebreaker += 1
            heappush(open_nodes, start_node) 
            ...
            while True: # loop until target point has been reached
                ...
                if len(open_nodes) == 0:
                    if carPositions == {}:
                        ...
                        broken = True
                        route2 = carPositions[vehicle.id]["Routes"]
                        route2 = [((round(x[0]), round(x[1])), x[2]) for x in route2]
                        routes[vehicle.id] = route2 # already inclusive of the second destination
                        break
                ...
                # If destination is same as current position (by chance) then skip this vehicle
                if road == vehicle.secondDestinationRoad and position == vehicle.secondDestinationPosition:
                    break

                # If destination is on the same road in front of the current position then calculate single instruction
                if road == vehicle.secondDestinationRoad and position < vehicle.secondDestinationPosition:
                    previous_node[(vehicle.secondDestinationRoad.roadID, vehicle.secondDestinationPosition)] = (road.roadID, position)
                    break
            ...
            if broken:
                continue
            # Initiate a list that stores the sequence of (next real position, road ID) for the vehicle
            route2: list[tuple[tuple[float, float], int]] = [] 

            # Initiate traceback variables
            current_roadID, current_position = vehicle.secondDestinationRoad.roadID, vehicle.secondDestinationPosition
            ...
            while (current_roadID, current_position) in previous_node:
                ...
                # Append new instruction
                newRealPos = getRealPositionOnRoad(landscape.roads[current_roadID], current_position)
                if len(route2) > 0 and newRealPos == route2[-1][0]:
                    pass # skip dupe coords in double intersections
                else:
                    route2.append((newRealPos, current_roadID))
            ...
            routes[vehicle.id] = route + route2
...
def computeAutoflowVehicleRoutes(autoflow_vehicles: list[Vehicle], landscape: Landscape, MAX_ROAD_SPEED_MPS: float, receiveNewDests, carPositions = {}) -> list[list[tuple[float, float]]]:
    """
...
def recalculateRoutes(carPositions, landscape : Landscape, vehicles : list[Vehicle], MAX_ROAD_SPEED_MPS, update_interval : int, receiveNewDests, roadBlockage):
    """
    ...
    # 1. randomly close and reopen roads
    # 0.25% chance of road being closed and 2% chance of road being opened (expected time to reopen: 50s)

    if roadBlockage == 1:
        for road in landscape.roads:
            chance = random.uniform(0, 1)
            if road.open and chance < 0.0025:
                road.open = False
                print(f"Road {road.roadID} closed")
            elif not road.open and chance < 0.02:
                road.open = True
                print(f"Road {road.roadID} opened")
    ...
    for id, data in carPositions.items():
        ...
        # if it has already reached dest1, make dest2 dest1 and find a new dest2
        if vehicle.destinationRealPosition not in route and receiveNewDests == 1:
            vehicle.setDestination(vehicle.secondDestinationRoad, vehicle.secondDestinationPosition)
            newSecondDest = random.choice(landscape.availableEnds)
            vehicle.setSecondDestination(*newSecondDest)
    ...
    newRoutes = computeRoutes(selfish, autoflow, landscape, MAX_ROAD_SPEED_MPS, receiveNewDests, carPositions=carPositions)
